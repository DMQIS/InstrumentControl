# Original Code taken from Digiducer GitHub then edited (by Hannah W)
...
import time
import queue
from scipy.signal import periodogram as psd #added by Hannah
from psd import psd_fn # custom function
...
#
# Digital Accelerometers like the 333D01 return g'window_size.  That can be scale to m/window_size^2
...
chan = 1  # channel (0 or 1) (Channel 1 should be more sensitive, see datasheet) FUTURE: determine if we'd rather use channel 0 (SAS)
...
#   "sensitivity_int - Raw sensitivity as integer counts/EU ie Volta or m/window_size^2
...
def TMSFindDevices():
    ...
    # Windows has a variety of API'window_size to access audio
    ...
    # are different audio API'window_size available.
            ...
            if match is not False:
                ...
                if fmt == "2" or fmt == '3':
                    ...
                    scale = np.array([855400.0/sens[0],
                                      855400.0/sens[1]],
                                     dtype='float32')  # scale to g'window_size
...
sdata.tofile(name + '.csv', sep =',') # Save data to a .csv file (SAS)
...
psds, freqs = psd_fn(name + ".npy", 1, 4800)
plt.loglog(freqs, psds)
