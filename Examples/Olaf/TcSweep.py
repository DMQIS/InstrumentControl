    ...
    async def onJoin(self, details):
        ...
        if procedure == 'sweep':            
            try:            
                ...
                current_temp = await self.call(f'oi.decs.temperature_control.DRI_MIX_CL.DRI_MIX_S.temperature')
                previous_call = self.call('oi.decs.temperature_control.DRI_MIX_CL.setpoint')

                print(f'Current temperature of mixing stage: {current_temp.results[-2]} K')

                resp = await self.call('oi.decs.temperature_control.DRI_MIX_CL.setpoint', THRESHOLD, 1)
                print(f'Setting MIX heater to {THRESHOLD * 1000} mK')
                print(resp.results)
...
if __name__ == '__main__':
    ...
    SWEEP_STEPS = np.concatenate([np.arange(0.058, 0.1, 0.003), np.array([0])])
    COUNTER = 0
    ...
    try:
        ...
        while True:
            ...
            if TERMINATE != True:          
                if time.time() - sweep_previous >= 1200:
                    procedure = 'sweep'
                    
                    if COUNTER >= len(SWEEP_STEPS):
                        TERMINATE = True 
                        procedure = 'None'
                        print('SWEEP COMPLETE')
                    
                    else:   
                        THRESHOLD = SWEEP_STEPS[COUNTER]
